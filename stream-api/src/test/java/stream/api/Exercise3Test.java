...
public class Exercise3Test extends ClassicOnlineStore {
    ...
    public void howManyItemsWanted() {
         ...
         */
        long sum = customerList.stream().flatMap(cus -> cus.getWantToBuy().stream()).count();
    ...
    public void richestCustomer() {
        ...
        Optional<Integer> richestCustomer = customerList.stream().map(cus -> cus.getBudget()).max(Comparator.naturalOrder());//With comparatore = Comparator.naturalOrder()
